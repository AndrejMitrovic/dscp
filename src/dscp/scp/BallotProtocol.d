 ...
 */
class BallotProtocolT (NodeID, Hash, Value, Signature, alias Set, alias makeSet, alias getHashOf, alias hashPart, alias duplicate, Logger)
{
        ...
        bool self)
    {
        ...
        {
            if (self)
                this.log.info("not sane statement from self, skipping   e: %s",
        ...
        {
            if (self)
                this.log.error("stale statement from self, skipping   e: %s",
                    this.mSlot.getSCP().envToStr(envelope));
            else
                this.log.trace("stale statement, skipping  i: %s",
        ...
        if (validationRes == ValidationLevel.kInvalidValue)
        {
            ...
            if (self)
                this.log.error("invalid value from self, skipping   e: %s",
                    this.mSlot.getSCP().envToStr(envelope));
            else
                this.log.trace("invalid value  i: %s", this.mSlot.getSlotIndex());
        ...
        if (this.mPhase == SCPPhase.SCP_PHASE_EXTERNALIZE)
        {
            ...
            if (self)
                this.log.error("externalize statement with invalid value " ~
    ...
    private bool isStatementSane (ref const(SCPStatement) st, bool self)
    {
        ...
        {
            this.log.info("Invalid quorum set received: %s", reason.to!string);
            return false;
        }
        ...
        final switch (st.pledges.type)
        {
            case SCPStatementType.SCP_ST_PREPARE:
            {
                ...
                if (!isOK)
                    this.log.trace("Malformed PREPARE message");
            ...
            case SCPStatementType.SCP_ST_CONFIRM:
            {
                ...
                if (!isOK)
                    this.log.trace("Malformed CONFIRM message");
                return isOK;
            }
            ...
            case SCPStatementType.SCP_ST_EXTERNALIZE:
            {
                ...
                if (!isOK)
                    this.log.trace("Malformed EXTERNALIZE message");
    ...
    {
        this.log.trace("BallotProtocol.abandonBallot");
    ...
    public bool bumpState (ref const(Value) value, uint32 n)
    {
        ...
        this.log.trace("BallotProtocol.bumpState i: %s v: %s",
    ...
    private bool updateCurrentValue (ref const(SCPBallot) ballot)
    {
        ...
        else
        {
            ...
            else if (comp > 0)
            {
                ...
                // with this situation
                this.log.error("BallotProtocol.updateCurrentValue attempt to " ~
        ...
        if (updated)
            this.log.trace("BallotProtocol.updateCurrentValue updated");
    ...
    {
        this.log.trace("BallotProtocol.bumpToBallot i: %s b: %s",
    ...
    {
        this.log.trace("BallotProtocol.setPreparedAccept i: %s b: %s",
            this.mSlot.getSlotIndex(), this.mSlot.getSCP().ballotToStr(ballot));
    ...
    {
        this.log.trace("BallotProtocol.setPreparedConfirmed i: %s h: %s",
    ...
    {
        this.log.trace("BallotProtocol.setAcceptCommit i: %s new c: %s new h: %s",
    ...
    {
        this.log.trace("BallotProtocol.setConfirmCommit i: %s new c: %s new h: %s",
            this.mSlot.getSlotIndex(), this.mSlot.getSCP().ballotToStr(c),
            this.mSlot.getSCP().ballotToStr(h));
    ...
    private void advanceSlot (ref const(SCPStatement) hint)
    {
        ...
        this.log.trace("BallotProtocol.advanceSlot %s %s",
        ...
        this.log.trace("BallotProtocol.advanceSlot %s - exiting %s",
            this.mCurrentMessageLevel, this);
